...
namespace WanBoWebapp.operation
{
    public class WatchResultDal
    {
        ...
        {
            string sqlString = "insert into 基础_微信巡更 (分类记录ID,工作日期,班次ID,巡更点ID,到达经度,到达纬度,到达时间,巡更序次,单次实际时长_分)" +
                "select @分类记录ID,@工作日期,@班次ID,@巡更点ID,@到达经度,@到达纬度,@到达时间,@巡更序次,@单次实际时长_分";

            StatusReport sr = SqlHelper.Insert(sqlString,
                new SqlParameter("@分类记录ID", result.OfficeId),
