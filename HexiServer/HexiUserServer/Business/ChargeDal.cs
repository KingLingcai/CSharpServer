...
namespace HexiUserServer.Business
{
    ...
    {
        public static Charge[] GetCharges(string ztCode, string roomId, string userId)
        {
            string sqlString = 
            ...
            DataTable dt = SQLHelper.ExecuteQuery(sqlString,
                new SqlParameter("@占用者ID", userId),
                new SqlParameter("@房产单元ID", roomId),
            ...
            foreach(DataRow row in dt.Rows)
            {
                ChargeDetail cd = new ChargeDetail()
                {
                    ...
                    ChargedId = DataTypeHelper.GetIntValue(row["收款ID"]),
                    ReceiptId = DataTypeHelper.GetIntValue(row["收据ID"])
                };
                cdList.Add(cd);
            }
        ...
        public static StatusReport SetCharges(string datetime, string totalCharge, string roomName, string proprietorName, string[] chargeIds)
        {
            ...
            sr = SQLHelper.Update(updateString,
